...
For a live book whose Amazon UK destination has not been confirmed to work, set `amazonLinkEnabled: false` in its catalogue record. Its page and verified ASIN remain visible, while its Buy button is withheld. Remove that flag only after a normal browser confirms the exact UK product page opens to the matching title. The Season Planner link is temporarily withheld: the owner verified ASIN `B0HJ6HGVC4` (digit 6 after J) in the live Amazon UK Product Information reached from KDP Bookshelf, while the generated `/dp/` URL failed in mobile testing. The exact working KDP destination has not yet been captured.
...
- Independently open the 21 currently linked Amazon UK pages in a normal supported browser and compare the titles and ASINs. For the Season Planner, capture the exact working UK URL from KDP Bookshelf’s “View on Amazon”, confirm it opens the matching title with ASIN `B0HJ6HGVC4`, then use that exact destination before restoring its Buy button. Amazon rejects automated sessions, so the working URL could not be independently captured here.
