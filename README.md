...
For a live book whose Amazon UK destination has not been confirmed to work, set `amazonLinkEnabled: false` in its catalogue record. Its page and verified ASIN remain visible, while its Buy button is withheld. Remove that flag only after a normal browser confirms the exact UK product page opens to the matching title. The Season Planner link is temporarily withheld pending that check; its latest KDP Bookshelf screenshot shows `B0HJGHGVC4` (letter G after J), while an older control report transcribed `B0HJ6HGVC4` (digit 6).
...
- Independently open the 21 currently linked Amazon UK pages in a normal supported browser and compare the titles and ASINs. For the Season Planner, verify `https://www.amazon.co.uk/dp/B0HJGHGVC4` opens the matching title before restoring its Buy button. Amazon rejects automated sessions, so product-page reachability was not confirmed here.
